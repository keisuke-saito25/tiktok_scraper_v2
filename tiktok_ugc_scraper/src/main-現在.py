...
def _parse_int_relaxed(v) -> int | None:
    """'66,600' や 'ＵＧＣ: 8030', '２０６６００' 等から整数だけ抜き出す。"""
    if v is None:
        return None
    s = str(v)
    m = _INT_RX.search(s)
...
def apply_mode(target_xlsx: Path, csv_inputs: list[str]):
    ...
    取り込み堅牢版:
      ...
      3) 取り込み時は utf-8-sig（BOM）で読む
      4) ugc_count は “緩和パース”。非数値はスキップ
      5) URL が空なら補完して以後の一致を安定化
    """
                ...
                with open(csv_path, newline="", encoding="utf-8-sig") as f:
                    ...
                    for row in reader:
                        ...
                        if r is None:
                            ...
                            if r is not None and (url is None or _canonical_url(url) not in url_index):
                                name_hit += 1
